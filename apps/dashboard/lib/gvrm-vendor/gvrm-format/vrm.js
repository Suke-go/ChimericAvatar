        ...
        this.loadingPromise = new Promise((resolve, reject) => {
            loader.load(
                ...
                async (gltf) => {
                    ...
                    // Local fork: do NOT call VRMUtils.removeUnnecessaryVertices /
                    // removeUnnecessaryJoints here. These reorder + drop vertices and
                    // bones (via three-vrm 3.x's combineSkeletons), which breaks the
                    // server-side preprocess assumption that splatVertexIndices /
                    // splatBoneIndices match the raw glTF order. Keeping the raw
                    // order preserves the contract between server preprocess and
                    // runtime shader anchor lookup. The performance hit is minor
                    // for a single avatar; correctness wins.
