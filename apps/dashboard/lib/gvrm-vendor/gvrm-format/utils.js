...
export function visualizeVRM(character, flag) {
  // Local fork: walk the WHOLE VRM scene and toggle every Mesh /
  // SkinnedMesh material. Without `removeUnnecessaryVertices/Joints`
  // (we disabled those upstream so server-side bindings can use the
  // raw glTF vertex/bone order), three-vrm leaves each glTF primitive
  // as its own SkinnedMesh — so the body alone yields 2 meshes and a
  // VRoid VRM ships ~9 (body × 2, face × 7 for face/hair/eyes/etc).
  // The original code only toggled the one at `skinnedMeshIndex`,
  // leaving the other ~8 visible — their default skin texture bled
  // through gaps in the splat coverage as glowing white blobs.
  // Iterating the whole subtree fixes that.
  const collected = [];
...
export function getPointsMeshCapsules(character) {
  // Local fork: prefer the explicitly resolved SkinnedMesh stashed by
  // initVRM. three-vrm 3.x's combineSkeletons can leave
  // `scene.children[skinnedMeshIndex]` pointing at a Group (no .geometry),
  // which would crash this function on the `.geometry.getAttribute` read
  // below. Falling back to the legacy lookup keeps compatibility with
  // older runtimes that don't run our discovery pass.
  const skinnedMesh = character.discoveredSkinnedMesh
