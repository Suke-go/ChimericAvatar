...
public sealed class PlyProperty
{
    ...
    public bool IsList;
    public string ListCountType = string.Empty;
    public string ListItemType = string.Empty;
}
...
public static class PlyParser
{
    public static PlyHeader ParseHeader(byte[] data)
    {
        if (data.Length < 3)
        {
            throw new InvalidDataException("PLY data too short.");
        }
        ...
        while (pos < data.Length)
        {
            ...
            if (eol < 0)
            {
                throw new InvalidDataException("PLY header is not newline terminated.");
            }

            int len = eol - pos;
            if (len > 0 && data[eol - 1] == (byte)'\r')
            {
                len--;
            }
            ...
            if (line == 1)
            {
                if (text != "ply")
                {
                    throw new InvalidDataException("PLY missing 'ply' magic.");
                }

                continue;
            }

            if (text.Length == 0 || text.StartsWith("comment ", StringComparison.Ordinal) || text.StartsWith("obj_info ", StringComparison.Ordinal))
            {
                continue;
            }
            ...
            {
                string[] parts = text.Split(' ', StringSplitOptions.RemoveEmptyEntries);
                current = new PlyElement { Name = parts[1], Count = int.Parse(parts[2], System.Globalization.CultureInfo.InvariantCulture) };
                header.Elements.Add(current);
            }
            else if (text.StartsWith("property ", StringComparison.Ordinal))
            {
                if (current == null)
                {
                    throw new InvalidDataException("Property declared before any element.");
                }
    ...
    public static int FixedStride(PlyElement element)
    {
        int total = 0;
        foreach (PlyProperty property in element.Properties)
        {
            if (property.IsList)
            {
                throw new InvalidDataException("List property in fixed-stride element.");
            }

            total += ScalarSize(property.Type);
        }
